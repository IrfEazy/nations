...
4. **Verify filter parameters use safe keywords ONLY**: Check all `[tileFilter]`, `[buildingFilter]`, and `[terrainFilter]` values in uniques against the Safe Keyword Substitution Table in copilot-instructions.md:
   - Safe keyword constants (e.g., `Friendly Land`, `Rough terrain`, `Water`, `Culture`) → ✅ always valid
   - Base-game terrain names (e.g., `Hill`, `Coast`, `Forest`) → ❌ **replace with safe keyword** (e.g., `Rough terrain`, `Water`)
   - Base-game building names (e.g., `Courthouse`, `Harbor`) → ❌ **redesign the unique** to use a stat keyword or different bonus
   - Unrecognized values → ❌ probable error — fix the filter value
5. Verify the new nation is balanced relative to existing ones
